...
public class BasicDebugEvent implements DebugEvent {
  ...
  public BasicDebugEvent(Type type, ThreadProxy thread) {
    this.type = type;
    this.thread = thread;
  }
  ...
  public static BasicDebugEvent newAccessViolationEvent(ThreadProxy thread,
                                                        ...
                                                        boolean wasWrite,
                                                        Address addr) {
    BasicDebugEvent ev = newPCEvent(Type.ACCESS_VIOLATION, thread, pc);
  ...
  public static BasicDebugEvent newUnknownEvent(ThreadProxy thread, String detail) {
    BasicDebugEvent ev = new BasicDebugEvent(Type.UNKNOWN, thread);
  ...
  private static BasicDebugEvent newAddressEvent(Type type, ThreadProxy thread, Address addr) {
    BasicDebugEvent ev = new BasicDebugEvent(type, thread);
    ev.setAddress(addr);
    return ev;
  }

  private static BasicDebugEvent newPCEvent(Type type, ThreadProxy thread, Address pc) {
    BasicDebugEvent ev = new BasicDebugEvent(type, thread);
    ev.setPC(pc);
    return ev;
  }
