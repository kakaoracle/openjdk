...
public class BasicCompoundType extends sun.jvm.hotspot.debugger.cdbg.basic.BasicType implements CompoundType {
  private sun.jvm.hotspot.debugger.cdbg.basic.CompoundTypeKind kind;
  private List baseClasses;
  private List fields;

  public BasicCompoundType(String name, int size, sun.jvm.hotspot.debugger.cdbg.basic.CompoundTypeKind kind) {
    this(name, size, kind, 0);
  }

  private BasicCompoundType(String name, int size, sun.jvm.hotspot.debugger.cdbg.basic.CompoundTypeKind kind, int cvAttributes) {
  ...
  public boolean isClass()  { return (kind == sun.jvm.hotspot.debugger.cdbg.basic.CompoundTypeKind.CLASS); }
  public boolean isStruct() { return (kind == sun.jvm.hotspot.debugger.cdbg.basic.CompoundTypeKind.STRUCT); }
  ...
  Type resolveTypes(BasicCDebugInfoDataBase db, ResolveListener listener) {
    ...
    if (baseClasses != null) {
      for (Iterator iter = baseClasses.iterator(); iter.hasNext(); ) {
        sun.jvm.hotspot.debugger.cdbg.basic.BasicBaseClass b = (BasicBaseClass) iter.next();
        b.resolveTypes(this, db, listener);
      }
    }
    if (fields != null) {
      for (Iterator iter = fields.iterator(); iter.hasNext(); ) {
        sun.jvm.hotspot.debugger.cdbg.basic.BasicField b = (BasicField) iter.next();
        b.resolveTypes(this, db, listener);
      }
    }
    return this;
  ...
  public void iterateObject(Address a, ObjectVisitor v, FieldIdentifier f) {
    ...
    if (f == null) {
      ...
      for (int i = 0; i < getNumFields(); i++) {
        ...
        sun.jvm.hotspot.debugger.cdbg.basic.BasicType fieldType = (BasicType) field.getType();
