...
public class CodeCache {
  ...
  private static synchronized void initialize(TypeDataBase db) {
    ...
    virtualConstructor.addMapping("nmethod", sun.jvm.hotspot.code.NMethod.class);
  ...
  public sun.jvm.hotspot.code.NMethod scavengeRootMethods() {
    return (sun.jvm.hotspot.code.NMethod) VMObjectFactory.newObject(sun.jvm.hotspot.code.NMethod.class, scavengeRootNMethodsField.getValue());
  }
  ...
  /** When VM.getVM().isDebugging() returns true, this behaves like
      findBlobUnsafe */
  public sun.jvm.hotspot.code.CodeBlob findBlob(Address start) {
    sun.jvm.hotspot.code.CodeBlob result = findBlobUnsafe(start);
  ...
  public sun.jvm.hotspot.code.CodeBlob findBlobUnsafe(Address start) {
    sun.jvm.hotspot.code.CodeBlob result = null;

    try {
      result = (sun.jvm.hotspot.code.CodeBlob) virtualConstructor.instantiateWrapperFor(getHeap().findStart(start));
    }
  ...
  public sun.jvm.hotspot.code.NMethod findNMethod(Address start) {
    sun.jvm.hotspot.code.CodeBlob cb = findBlob(start);
    if (Assert.ASSERTS_ENABLED) {
      Assert.that(cb == null || cb.isNMethod(), "did not find an nmethod");
    }
    return (sun.jvm.hotspot.code.NMethod) cb;
  }

  public sun.jvm.hotspot.code.NMethod findNMethodUnsafe(Address start) {
    sun.jvm.hotspot.code.CodeBlob cb = findBlobUnsafe(start);
    if (Assert.ASSERTS_ENABLED) {
      Assert.that(cb == null || cb.isNMethod(), "did not find an nmethod");
    }
    return (NMethod) cb;
  ...
  public sun.jvm.hotspot.code.CodeBlob createCodeBlobWrapper(Address codeBlobAddr) {
    try {
      return (sun.jvm.hotspot.code.CodeBlob) virtualConstructor.instantiateWrapperFor(codeBlobAddr);
    }
    catch (Exception e) {
  ...
  public void iterate(CodeCacheVisitor visitor) {
    ...
    sun.jvm.hotspot.code.CodeBlob lastBlob = null;
