...
public class BytecodeLookupswitch extends Bytecode {
  ...
  public sun.jvm.hotspot.interpreter.LookupswitchPair pairAt(int i) {
    if (Assert.ASSERTS_ENABLED) {
      Assert.that(0 <= i && i < numberOfPairs(), "pair index out of bounds");
    }
    return new sun.jvm.hotspot.interpreter.LookupswitchPair(method, bci + alignedOffset(1 + (1 + i)*2*jintSize));
  }
