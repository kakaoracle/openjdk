...
public class RBTree {
  ...
  public void insertNode(sun.jvm.hotspot.utilities.RBNode x) {
    treeInsert(x);
    ...
    sun.jvm.hotspot.utilities.RBNode propagateStart = x;

    // Loop invariant: x has been updated.
    while ((x != root) && (x.getParent().getColor() == RBColor.RED)) {
      if (x.getParent() == x.getParent().getParent().getLeft()) {
        sun.jvm.hotspot.utilities.RBNode y = x.getParent().getParent().getRight();
        ...
        sun.jvm.hotspot.utilities.RBNode y = x.getParent().getParent().getLeft();
        if ((y != null) && (y.getColor() == RBColor.RED)) {
          // Case 1
          if (DEBUGGING && REALLY_VERBOSE) {
  ...
  public void deleteNode(sun.jvm.hotspot.utilities.RBNode z) {
  ...
  private void treeInsert(sun.jvm.hotspot.utilities.RBNode z) {
    sun.jvm.hotspot.utilities.RBNode y = null;
    sun.jvm.hotspot.utilities.RBNode x = root;
  ...
  private sun.jvm.hotspot.utilities.RBNode treeSuccessor(sun.jvm.hotspot.utilities.RBNode x) {
    if (x.getRight() != null) {
      return treeMinimum(x.getRight());
    }
    sun.jvm.hotspot.utilities.RBNode y = x.getParent();
  ...
  private sun.jvm.hotspot.utilities.RBNode treeMinimum(sun.jvm.hotspot.utilities.RBNode x) {
  ...
  private boolean leftRotate(sun.jvm.hotspot.utilities.RBNode x) {
    // Set y.
    sun.jvm.hotspot.utilities.RBNode y = x.getRight();
  ...
  /** Returns true if updates must continue propagating up the tree */
  private boolean rightRotate(sun.jvm.hotspot.utilities.RBNode y) {
    // Set x.
    sun.jvm.hotspot.utilities.RBNode x = y.getLeft();
  ...
  /** Restores red-black property to tree after splicing out node
      ...
      must be specified. */
  private void deleteFixup(sun.jvm.hotspot.utilities.RBNode x, sun.jvm.hotspot.utilities.RBNode xParent) {
    while ((x != root) && ((x == null) || (x.getColor() == RBColor.BLACK))) {
      if (x == xParent.getLeft()) {
        ...
        sun.jvm.hotspot.utilities.RBNode w = xParent.getRight();
        if (DEBUGGING) {
          if (w == null) {
        ...
        // reason is not obvious from simply examining the code; it
        // comes about because of properties of the red-black tree.
        sun.jvm.hotspot.utilities.RBNode w = xParent.getLeft();
        if (DEBUGGING) {
          if (w == null) {
            throw new RuntimeException("x's sibling should not be null");
          }
  ...
  private int verifyFromNode(sun.jvm.hotspot.utilities.RBNode node) {
  ...
  /** Debugging */
  private void printFromNode(sun.jvm.hotspot.utilities.RBNode node, PrintStream tty, int indentDepth) {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < treeSize; i++) {
      ...
      try {
        tree.insertNode(new sun.jvm.hotspot.utilities.RBNode(val));
        if ((i > 0) && (i % 100 == 0)) {
          System.err.print(i + "...");
          System.err.flush();
        }
    ...
    for (int i = 0; i < treeSize; i++) {
      ...
      sun.jvm.hotspot.utilities.RBNode xParent = null;
      sun.jvm.hotspot.utilities.RBNode x = tree.getRoot();
      int depth = 0;
      while (x != null) {
