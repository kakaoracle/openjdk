...
public class JNIHandleBlock extends VMObject {
  ...
  public String toString() {
    ...
    Address handleEnd = addr.addOffsetTo(handlesField.getOffset() + top() * sun.jvm.hotspot.runtime.VM.getVM().getOopSize());
  ...
  private Address getOopHandleAddress(int x) {
    ...
    Address oopAddr = addr.addOffsetTo(handlesField.getOffset() + x * sun.jvm.hotspot.runtime.VM.getVM().getOopSize());
    OopHandle handle = oopAddr.getOopHandleAt(0);
    if (sun.jvm.hotspot.runtime.VM.getVM().getUniverse().isInReserved(handle) && !VM.getVM().getJNIHandles().isDeletedHandle(handle)) {
