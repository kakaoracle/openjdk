...
package hotspot.agent.src.share.classes.sun.jvm.hotspot.ui;
...
public class AnnotatedMemoryPanel extends JPanel {
  ...
  public synchronized void paintComponent(Graphics g) {
    ...
    int addrWidth;
    {
      Rectangle2D bounds = sun.jvm.hotspot.ui.GraphicsUtilities.getStringBounds(unmappedAddrString, g);
    ...
    for (Iterator iter = visibleAnnotations.iterator(); iter.hasNext(); ) {
      sun.jvm.hotspot.ui.Annotation anno   = (sun.jvm.hotspot.ui.Annotation) iter.next();
  ...
  public synchronized void addAnnotation(sun.jvm.hotspot.ui.Annotation annotation) {
  ...
  private void layoutAnnotations(java.util.List va,
    ...
    sun.jvm.hotspot.ui.Annotation constraintAnnotation = null;
    // The first constraint annotation
    sun.jvm.hotspot.ui.Annotation firstConstraintAnnotation = null;
    ...
    for (Iterator iter = va.iterator(); iter.hasNext(); ) {
      sun.jvm.hotspot.ui.Annotation anno = (sun.jvm.hotspot.ui.Annotation) ((IntervalNode) iter.next()).getData();

      // Search forward for this one
      boolean found = false;
      for (int i = searchIndex; i < visibleAnnotations.size(); i++) {
        sun.jvm.hotspot.ui.Annotation el = (sun.jvm.hotspot.ui.Annotation) visibleAnnotations.get(i);
    ...
    if (firstConstraintAnnotation != null) {
      ...
      for (int i = deferredIndex; i >= 0; i--) {
        sun.jvm.hotspot.ui.Annotation anno = (sun.jvm.hotspot.ui.Annotation) newAnnos.get(i);
  ...
  /** Lays out the given annotation before the optional constraint
      ...
      is specified. */
  private void layoutBefore(sun.jvm.hotspot.ui.Annotation anno, sun.jvm.hotspot.ui.Annotation constraintAnno,
                            Graphics g, int x,
  ...
  /** Lays out the given annotation after the optional constraint
      annotation, obeying constraints imposed by that annotation if it
      is specified. */
  private void layoutAfter(sun.jvm.hotspot.ui.Annotation anno, sun.jvm.hotspot.ui.Annotation constraintAnno,
                           Graphics g, int x,
                           Address startAddr, int lineHeight) {
    anno.computeWidthAndHeight(g);
    // Color
  ...
  public static void main(String[] args) {
    ...
    anno.addAnnotation(new sun.jvm.hotspot.ui.Annotation(debugger.parseAddress("0x80000000"),
                                      debugger.parseAddress("0x80000040"),
                                      "Stack Frame for \"foo\""));
    anno.addAnnotation(new sun.jvm.hotspot.ui.Annotation(debugger.parseAddress("0x80000010"),
