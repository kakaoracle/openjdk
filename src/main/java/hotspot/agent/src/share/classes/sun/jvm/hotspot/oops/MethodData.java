...
public class MethodData extends Metadata {
  ...
  static final int dsReasonMask   = sun.jvm.hotspot.oops.DataLayout.trapMask >> 1;
  static final int dsRecompileBit = sun.jvm.hotspot.oops.DataLayout.trapMask - dsReasonMask;
  ...
  private static synchronized void initialize(TypeDataBase db) throws WrongTypeException {
    ...
    size           = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_size"), 0);
    ...
    dataSize     = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_data_size"), 0);
  ...
  public sun.jvm.hotspot.oops.Method getMethod() {
    return (sun.jvm.hotspot.oops.Method) method.getValue(this);
  }

  public void printValueOn(PrintStream tty) {
    sun.jvm.hotspot.oops.Method m = getMethod();
  ...
  ProfileData dataAt(int dataIndex) {
    ...
    sun.jvm.hotspot.oops.DataLayout dataLayout = new sun.jvm.hotspot.oops.DataLayout(this, dataIndex + (int)data.getOffset());

    switch (dataLayout.tag()) {
    case sun.jvm.hotspot.oops.DataLayout.noTag:
    default:
      throw new InternalError(dataIndex + " " + dataSize() + " " + dataLayout.tag());
    case sun.jvm.hotspot.oops.DataLayout.bitDataTag:
      return new BitData(dataLayout);
    case sun.jvm.hotspot.oops.DataLayout.counterDataTag:
      return new CounterData(dataLayout);
    case sun.jvm.hotspot.oops.DataLayout.jumpDataTag:
      return new JumpData(dataLayout);
    case sun.jvm.hotspot.oops.DataLayout.receiverTypeDataTag:
      return new ReceiverTypeData(dataLayout);
    case sun.jvm.hotspot.oops.DataLayout.virtualCallDataTag:
      return new VirtualCallData(dataLayout);
    case sun.jvm.hotspot.oops.DataLayout.retDataTag:
      return new RetData(dataLayout);
    case sun.jvm.hotspot.oops.DataLayout.branchDataTag:
  ...
  int mileageOf(sun.jvm.hotspot.oops.Method method) {
    long mileage = 0;
  ...
  public void dumpReplayData(PrintStream out) {
    ...
    sun.jvm.hotspot.oops.Klass holder = method.getMethodHolder();
      ...
      for ( ; isValid(pdata); pdata = nextData(pdata)) {
        if (pdata instanceof ReceiverTypeData) {
          ...
          for (int i = 0; i < vdata.rowLimit(); i++) {
            sun.jvm.hotspot.oops.Klass k = vdata.receiver(i);
            if (k != null) {
              if (round == 0) count++;
              else out.print(" " +
                             (dpToDi(vdata.dp() +
