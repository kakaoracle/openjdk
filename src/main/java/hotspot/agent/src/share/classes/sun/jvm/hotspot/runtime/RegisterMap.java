...
public abstract class RegisterMap implements Cloneable {
  ...
  protected RegisterMap(sun.jvm.hotspot.runtime.JavaThread thread, boolean updateMap) {
    this.thread    = thread;
  ...
  public Address getLocation(sun.jvm.hotspot.runtime.VMReg reg) {
    int i = reg.getValue();
  ...
  public void setLocation(sun.jvm.hotspot.runtime.VMReg reg, Address loc) {
    int i = reg.getValue();
    int index = i / locationValidTypeSize;
    if (Assert.ASSERTS_ENABLED) {
      Assert.that(0 <= i && i < regCount, "sanity check");
  ...
  public void printOn(PrintStream tty) {
    ...
    for (int i = 0; i < location.length; i++) {
      Address src = getLocation(new sun.jvm.hotspot.runtime.VMReg(i));
      if (src != null) {
