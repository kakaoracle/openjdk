...
public class LivenessAnalysis {
  ...
  private static LivenessPathList computeAllLivenessPaths(Oop target, boolean trimPathsThroughPopularObjects) {
    ...
    {
      sun.jvm.hotspot.utilities.LivenessPath path = new sun.jvm.hotspot.utilities.LivenessPath();
    ...
    while (true) {
      ...
      sun.jvm.hotspot.utilities.LivenessPath path = null;

      for (int i = list.size() - 1; i >= 0; i--) {
        sun.jvm.hotspot.utilities.LivenessPath tmp = list.get(i);
        if (!tmp.isComplete()) {
          path = tmp;
          break;
        }
        ...
        if (nextPtrs != null) {
          ...
          for (Iterator iter = nextPtrs.iterator(); iter.hasNext(); ) {
            ...
            if ((nextElement.isRoot() && (visitedRoots.get(nextElement) == null)) ||
              ...
              sun.jvm.hotspot.utilities.LivenessPath nextPath = path.copy();
