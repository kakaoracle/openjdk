...
public class PageCache {
  ...
  public synchronized byte[] getData(long startAddress, long numBytes)
    ...
    while (numBytes > 0) {
      ...
      // Look up this page
      sun.jvm.hotspot.debugger.Page page = checkPage(getPage(pageBaseAddress), startAddress);
  ...
  private sun.jvm.hotspot.debugger.Page lruList; // Most recently fetched page, or null

  /** Page fetcher plus LRU functionality */
  private sun.jvm.hotspot.debugger.Page getPage(long pageBaseAddress) {
    ...
    sun.jvm.hotspot.debugger.Page page = (sun.jvm.hotspot.debugger.Page) addressToPageMap.get(key);
    if (page == null) {
      ...
      if (enabled) {
        ...
        if (Assert.ASSERTS_ENABLED) {
          Assert.that(page == (sun.jvm.hotspot.debugger.Page) addressToPageMap.get(pageBaseAddress),
                      "must have found page in cache!");
        }
        addPageToList(page);
        // See whether eviction of oldest is necessary
        if (numPages == maxNumPages) {
          sun.jvm.hotspot.debugger.Page evictedPage = lruList.getPrev();
  ...
  private sun.jvm.hotspot.debugger.Page checkPage(sun.jvm.hotspot.debugger.Page page, long startAddress) {
    if (!page.isMapped()) {
  ...
  private int countPages() {
    sun.jvm.hotspot.debugger.Page page = lruList;
    int num = 0;
    if (page == null) {
      return num;
    }
  ...
  private void flushPage(long pageBaseAddress) {
    long key = pageBaseAddress;
    sun.jvm.hotspot.debugger.Page page = (sun.jvm.hotspot.debugger.Page) addressToPageMap.remove(key);
    if (page != null) {
      removePageFromList(page);
    }
  }
  ...
  private void addPageToList(sun.jvm.hotspot.debugger.Page page) {
    if (lruList == null) {
      lruList = page;
      page.setNext(page);
      page.setPrev(page);
