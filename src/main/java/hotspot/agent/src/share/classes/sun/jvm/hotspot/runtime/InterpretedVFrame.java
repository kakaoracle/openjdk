...
public class InterpretedVFrame extends JavaVFrame {
  ...
  public StackValueCollection getLocals() {
    ...
    for(int i = 0; i < length; i++) {
      ...
      if (oopMask.isOop(i)) {
        // oop value
        sv = new sun.jvm.hotspot.runtime.StackValue(addr.getOopHandleAt(0), 0);
      } else {
        // integer
        // Fetch a signed integer the size of a stack slot
        sv = new sun.jvm.hotspot.runtime.StackValue(addr.getCIntegerAt(0, sun.jvm.hotspot.runtime.VM.getVM().getAddressSize(), false));
      }
      result.add(sv);
    }
