...
public class BytecodeStream {
  ...
  public int next() {
    ...
    if (isLastBytecode()) {
      ...
      code = sun.jvm.hotspot.interpreter.Bytecodes._illegal;
    } else {
      // get bytecode
      int rawCode = sun.jvm.hotspot.interpreter.Bytecodes.codeAt(_method, _bci);
      code = 0; // Make javac happy
      try {
        code = sun.jvm.hotspot.interpreter.Bytecodes.javaCode(rawCode);
      ...
      //
      int l = sun.jvm.hotspot.interpreter.Bytecodes.lengthFor(code);
      if (l == 0) l = sun.jvm.hotspot.interpreter.Bytecodes.lengthAt(_method, _bci);
      _next_bci  += l;
      if (Assert.ASSERTS_ENABLED) {
      ...
      if (code == sun.jvm.hotspot.interpreter.Bytecodes._wide) {
        code = _method.getBytecodeOrBPAt(_bci + 1);
        _is_wide = true;
      }
      if (Assert.ASSERTS_ENABLED) {
        Assert.that(sun.jvm.hotspot.interpreter.Bytecodes.isJavaCode(code), "sanity check");
      }
    }
    _code = code;
    return _code;
  ...
  public boolean isActiveBreakpoint() { return sun.jvm.hotspot.interpreter.Bytecodes.isActiveBreakpointAt(_method, _bci); }
