...
public class VFrame {
  ...
  protected VFrame(sun.jvm.hotspot.runtime.Frame f, RegisterMap regMap, JavaThread thread) {
    this.regMap = (RegisterMap) regMap.clone();

    if (f != null) {
      // the frame is null if we create a deoptimizedVFrame from a vframeArray
      fr = (sun.jvm.hotspot.runtime.Frame) f.clone();
    }
  ...
  public static VFrame newVFrame(sun.jvm.hotspot.runtime.Frame f, RegisterMap regMap, JavaThread thread, boolean unsafe, boolean mayBeImprecise) {
    ...
    if (!sun.jvm.hotspot.runtime.VM.getVM().isCore()) {
      CodeBlob cb;
      if (unsafe) {
        cb = sun.jvm.hotspot.runtime.VM.getVM().getCodeCache().findBlobUnsafe(f.getPC());
      } else {
        cb = sun.jvm.hotspot.runtime.VM.getVM().getCodeCache().findBlob(f.getPC());
      }
      ...
      if (cb != null) {
        if (cb.isNMethod()) {
          ...
          if (mayBeImprecise || sun.jvm.hotspot.runtime.VM.getVM().isDebugging()) {
        ...
        if (f.isRuntimeFrame()) {
          ...
          sun.jvm.hotspot.runtime.Frame s = f.sender(tempMap);
  ...
  /** Factory method for creating vframes. This is equivalent to
      ...
      flags set to false. */
  public static VFrame newVFrame(sun.jvm.hotspot.runtime.Frame f, RegisterMap regMap, JavaThread thread) {
    return newVFrame(f, regMap, thread, false, false);
  }

  /** Accessors */
  public sun.jvm.hotspot.runtime.Frame getFrame()       { return fr;     }
  ...
  public VFrame sender(boolean mayBeImprecise) {
    ...
    if (s == null) {
      Assert.that(sun.jvm.hotspot.runtime.VM.getVM().getCPU().equals("ia64"), "Only ia64 should have null here");
      return null;
    }
    if (s.isFirstFrame()) {
      return null;
    }
    return VFrame.newVFrame(s, tempMap, getThread(), sun.jvm.hotspot.runtime.VM.getVM().isDebugging(), mayBeImprecise);
  }
