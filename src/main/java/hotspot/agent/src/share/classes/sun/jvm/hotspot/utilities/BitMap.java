...
public class BitMap {
  ...
  public boolean at(int offset) {
    if (sun.jvm.hotspot.utilities.Assert.ASSERTS_ENABLED) {
      sun.jvm.hotspot.utilities.Assert.that(offset>=0 && offset < size(), "BitMap index out of bounds");
    }
    return sun.jvm.hotspot.utilities.Bits.isSetNthBit(wordFor(offset), offset % bitsPerWord);
  }
  ...
  public void atPut(int offset, boolean value) {
    ...
    if (value) {
      data[index] = sun.jvm.hotspot.utilities.Bits.setNthBit(data[index], pos);
    } else {
      data[index] = sun.jvm.hotspot.utilities.Bits.clearNthBit(data[index], pos);
    }
  }
  ...
  public void clear() {
    for (int i = 0; i < sizeInWords(); i++) {
      data[i] = sun.jvm.hotspot.utilities.Bits.NoBits;
    }
  }
  ...
  public boolean setUnion(BitMap other) {
    if (sun.jvm.hotspot.utilities.Assert.ASSERTS_ENABLED) {
      sun.jvm.hotspot.utilities.Assert.that(size() == other.size(), "must have same size");
    }
