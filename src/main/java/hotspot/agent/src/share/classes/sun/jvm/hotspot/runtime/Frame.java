...
public abstract class Frame implements Cloneable {
  ...
  public CodeBlob cb() {
    return sun.jvm.hotspot.runtime.VM.getVM().getCodeCache().findBlob(getPC());
  }
  ...
  /** type testers */
  public boolean isInterpretedFrame()           { return sun.jvm.hotspot.runtime.VM.getVM().getInterpreter().contains(getPC()); }
  public boolean isJavaFrame() {
    if (isInterpretedFrame()) return true;
    if (!sun.jvm.hotspot.runtime.VM.getVM().isCore()) {
  ...
  public boolean isEntryFrame()                 { return sun.jvm.hotspot.runtime.VM.getVM().getStubRoutines().returnsToCallStub(getPC()); }
  public boolean isNativeFrame() {
    if (!sun.jvm.hotspot.runtime.VM.getVM().isCore()) {
      CodeBlob cb = sun.jvm.hotspot.runtime.VM.getVM().getCodeCache().findBlob(getPC());
  ...
  public boolean isCompiledFrame() {
    if (Assert.ASSERTS_ENABLED) {
      Assert.that(!sun.jvm.hotspot.runtime.VM.getVM().isCore(), "noncore builds only");
    }
    CodeBlob cb = sun.jvm.hotspot.runtime.VM.getVM().getCodeCache().findBlob(getPC());
  ...
  public Address   addressOfStackSlot(int slot)              { return getFP().addOffsetTo(slot * sun.jvm.hotspot.runtime.VM.getVM().getAddressSize()); }
  ...
  public Address addressOfInterpreterFrameLocal(int slot) {
    return addressOfInterpreterFrameLocals().getAddressAt(0).addOffsetTo(-slot * sun.jvm.hotspot.runtime.VM.getVM().getAddressSize());
  }
  ...
  public Address addressOfInterpreterFrameExpressionStackSlot(int slot) {
    return addressOfInterpreterFrameExpressionStack().addOffsetTo(-slot * sun.jvm.hotspot.runtime.VM.getVM().getAddressSize());
  }
  ...
  public Address oopMapRegToLocation(VMReg reg, RegisterMap regMap) {
    VMReg stack0 = sun.jvm.hotspot.runtime.VM.getVM().getVMRegImplInfo().getStack0();
    if (reg.lessThan(stack0)) {
      // If it is passed in a register, it got spilled in the stub frame.
      return regMap.getLocation(reg);
    } else {
      long spOffset = sun.jvm.hotspot.runtime.VM.getVM().getAddressSize() * reg.minus(stack0);
  ...
  public void oopsDo(AddressVisitor oopVisitor, RegisterMap map) {
    ...
    } else if (sun.jvm.hotspot.runtime.VM.getVM().getCodeCache().contains(getPC())) {
  ...
  private static class InterpVisitor implements OopMapVisitor {
    ...
    public void visitDerivedOopLocation(Address baseOopAddr, Address derivedOopAddr) {
      if (sun.jvm.hotspot.runtime.VM.getVM().isClientCompiler()) {
        Assert.that(false, "should not reach here");
      } else if (sun.jvm.hotspot.runtime.VM.getVM().isServerCompiler() &&
                 sun.jvm.hotspot.runtime.VM.getVM().useDerivedPointerTable()) {
