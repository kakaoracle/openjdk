...
public class BytecodeLoadConstant extends Bytecode {
  ...
  public boolean hasCacheIndex() {
    ...
    return code() >= sun.jvm.hotspot.interpreter.Bytecodes.number_of_java_codes;
  }

  int rawIndex() {
    if (javaCode() == sun.jvm.hotspot.interpreter.Bytecodes._ldc)
      return getIndexU1();
    else
  ...
  public boolean isValid() {
    int jcode = javaCode();
    boolean codeOk = jcode == sun.jvm.hotspot.interpreter.Bytecodes._ldc || jcode == sun.jvm.hotspot.interpreter.Bytecodes._ldc_w ||
           jcode == sun.jvm.hotspot.interpreter.Bytecodes._ldc2_w;
    if (! codeOk) return false;

    ConstantTag ctag = method().getConstants().getTagAt(poolIndex());
    if (jcode == sun.jvm.hotspot.interpreter.Bytecodes._ldc2_w) {
