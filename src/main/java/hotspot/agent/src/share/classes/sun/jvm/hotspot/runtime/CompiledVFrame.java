...
public class CompiledVFrame extends JavaVFrame {
  ...
  public CompiledVFrame(sun.jvm.hotspot.runtime.Frame fr, RegisterMap regMap, JavaThread thread, ScopeDesc scope, boolean mayBeImprecise) {
    super(fr, regMap, thread);
    this.scope = scope;
    this.mayBeImprecise = mayBeImprecise;
    if (!sun.jvm.hotspot.runtime.VM.getVM().isDebugging()) {
  ...
  public NMethod getCode() {
    return sun.jvm.hotspot.runtime.VM.getVM().getCodeCache().findNMethod(fr.getPC());
  }
  ...
  public NMethod getCodeUnsafe() {
    return sun.jvm.hotspot.runtime.VM.getVM().getCodeCache().findNMethodUnsafe(fr.getPC());
  }
  ...
  public Method getMethod() {
    if (sun.jvm.hotspot.runtime.VM.getVM().isDebugging() && getScope() == null) {
  ...
  public List   getMonitors() {
    ...
    for (int i = 0; i < monitors.size(); i++) {
      ...
      sun.jvm.hotspot.runtime.StackValue ownerSV = createStackValue(ov); // it is an oop
      if (ov.isObject()) { // The owner object was scalar replaced
        ...
        result.add(new sun.jvm.hotspot.runtime.MonitorInfo(k, resolveMonitorLock(mv.basicLock()), mv.eliminated(), true));
      } else {
  ...
  public VFrame sender(boolean mayBeImprecise) {
    ...
    sun.jvm.hotspot.runtime.Frame f = (Frame) getFrame().clone();
    return (isTop()
              ? super.sender(false)
              : new CompiledVFrame(f, getRegisterMap(), getThread(), getScope().sender(), mayBeImprecise));
  }

  private sun.jvm.hotspot.runtime.StackValue createStackValue(ScopeValue sv) {
    ...
    if (sv.isLocation()) {
      ...
      if (loc.isIllegal()) return new sun.jvm.hotspot.runtime.StackValue();
      ...
      if (loc.holdsFloat()) {    // Holds a float in a double register?
        ...
        return new sun.jvm.hotspot.runtime.StackValue(Float.floatToIntBits(value) & 0xFFFFFFFF); // 64-bit high half is stack junk
        ...
        return new sun.jvm.hotspot.runtime.StackValue(valueAddr.getJLongAt(0) & 0xFFFFFFFF);
      } else if (loc.holdsNarrowOop()) {  // Holds an narrow oop?
        if (loc.isRegister() && sun.jvm.hotspot.runtime.VM.getVM().isBigEndian()) {
          ...
          return new sun.jvm.hotspot.runtime.StackValue(valueAddr.getCompOopHandleAt(sun.jvm.hotspot.runtime.VM.getVM().getIntSize()), 0);
        } else {
          return new sun.jvm.hotspot.runtime.StackValue(valueAddr.getCompOopHandleAt(0), 0);
        }
      } else if( loc.holdsOop() ) {  // Holds an oop?
        return new sun.jvm.hotspot.runtime.StackValue(valueAddr.getOopHandleAt(0), 0);
      } else if( loc.holdsDouble() ) {
        // Double value in a single stack slot
        return new sun.jvm.hotspot.runtime.StackValue(valueAddr.getJIntAt(0) & 0xFFFFFFFF);
      } else if(loc.holdsAddr()) {
        if (Assert.ASSERTS_ENABLED) {
          Assert.that(!sun.jvm.hotspot.runtime.VM.getVM().isServerCompiler(), "No address type for locations with C2 (jsr-s are inlined)");
        }
        // FIXME: not yet implemented (no access to safepoint state yet)
        return new sun.jvm.hotspot.runtime.StackValue(0);
      ...
      } else if (sun.jvm.hotspot.runtime.VM.getVM().isLP64() && loc.holdsLong()) {
        if ( loc.isRegister() ) {
          // Long value in two registers, high half in the first, low in the second
          return new sun.jvm.hotspot.runtime.StackValue(((valueAddr.getJLongAt(0) & 0xFFFFFFFF) << 32) |
                                ((valueAddr.getJLongAt(8) & 0xFFFFFFFF)));
        } else {
          // Long value in a single stack slot
          return new sun.jvm.hotspot.runtime.StackValue(valueAddr.getJLongAt(0));
        }
      ...
      return new sun.jvm.hotspot.runtime.StackValue(((ConstantIntValue) sv).getValue() & 0xFFFFFFFF);
    } else if (sv.isConstantOop()) {
      // constant oop
      return new sun.jvm.hotspot.runtime.StackValue(((ConstantOopReadValue) sv).getValue(), 0);
    } else if (sv.isConstantDouble()) {
      // Constant double in a single stack slot
      double d = ((ConstantDoubleValue) sv).getValue();
      return new sun.jvm.hotspot.runtime.StackValue(Double.doubleToLongBits(d) & 0xFFFFFFFF);
    } else if (VM.getVM().isLP64() && sv.isConstantLong()) {
      // Constant long in a single stack slot
      return new sun.jvm.hotspot.runtime.StackValue(((ConstantLongValue) sv).getValue() & 0xFFFFFFFF);
    } else if (sv.isObject()) {
      // Scalar replaced object in compiled frame
      return new sun.jvm.hotspot.runtime.StackValue(((ObjectValue)sv).getValue(), 1);
    }
  ...
  private sun.jvm.hotspot.runtime.BasicLock resolveMonitorLock(Location location) {
    if (Assert.ASSERTS_ENABLED) {
