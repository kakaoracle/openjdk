...
public class NMethod extends CodeBlob {
  ...
  public boolean isMethodHandleReturn(Address returnPc) {
    ...
    sun.jvm.hotspot.code.PCDesc pd = getPCDescAt(returnPc);
    if (pd == null)
      return false;
  ...
  public sun.jvm.hotspot.code.PCDesc getPCDescAt(Address pc) {
    ...
    for (Address p = scopesPCsBegin(); p.lessThan(scopesPCsEnd()); p = p.addOffsetTo(pcDescSize)) {
      sun.jvm.hotspot.code.PCDesc pcDesc = new sun.jvm.hotspot.code.PCDesc(p);
  ...
  public ScopeDesc getScopeDescAt(Address pc) {
    sun.jvm.hotspot.code.PCDesc pd = getPCDescAt(pc);
    if (Assert.ASSERTS_ENABLED) {
  ...
  public sun.jvm.hotspot.code.PCDesc getPCDescNearDbg(Address pc) {
    sun.jvm.hotspot.code.PCDesc bestGuessPCDesc = null;
    long bestDistance = 0;
    for (Address p = scopesPCsBegin(); p.lessThan(scopesPCsEnd()); p = p.addOffsetTo(pcDescSize)) {
      sun.jvm.hotspot.code.PCDesc pcDesc = new sun.jvm.hotspot.code.PCDesc(p);
      // In case pc is null
  ...
  sun.jvm.hotspot.code.PCDesc find_pc_desc(long pc, boolean approximate) {
    return find_pc_desc_internal(pc, approximate);
  }

  // Finds a PcDesc with real-pc equal to "pc"
  sun.jvm.hotspot.code.PCDesc find_pc_desc_internal(long pc, boolean approximate) {
    ...
    for (int step = (1 << (LOG2_RADIX*3)); step > 1; step >>= LOG2_RADIX) {
      while ((mid = lower.addOffsetTo(step * pcDescSize)).lessThan(upper)) {
        sun.jvm.hotspot.code.PCDesc m = new sun.jvm.hotspot.code.PCDesc(mid);
    ...
    sun.jvm.hotspot.code.PCDesc u = new sun.jvm.hotspot.code.PCDesc(upper);
  ...
  sun.jvm.hotspot.code.PCDesc pc_desc_at(long pc)   { return find_pc_desc(pc, false); }
  // pc_desc_near returns the first PCDesc at or after the givne pc.
  sun.jvm.hotspot.code.PCDesc pc_desc_near(long pc) { return find_pc_desc(pc, true); }

  // Return a the last scope in (begin..end]
  public ScopeDesc scope_desc_in(long begin, long end) {
    sun.jvm.hotspot.code.PCDesc p = pc_desc_near(begin+1);
  ...
  static boolean match_desc(sun.jvm.hotspot.code.PCDesc pc, int pc_offset, boolean approximate) {
    if (!approximate) {
      return pc.getPCOffset() == pc_offset;
    } else {
      sun.jvm.hotspot.code.PCDesc prev = new sun.jvm.hotspot.code.PCDesc(pc.getAddress().addOffsetTo(-pcDescSize));
  ...
  public ScopeDesc getScopeDescNearDbg(Address pc) {
    sun.jvm.hotspot.code.PCDesc pd = getPCDescNearDbg(pc);
  ...
  public Map/*<Address, PCDesc>*/ getSafepoints() {
    ...
    for (p = scopesPCsBegin(); p.lessThan(scopesPCsEnd());
       ...
       sun.jvm.hotspot.code.PCDesc pcDesc = new PCDesc(p);
