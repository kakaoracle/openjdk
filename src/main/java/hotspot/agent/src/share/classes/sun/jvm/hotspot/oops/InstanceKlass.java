...
public class InstanceKlass extends sun.jvm.hotspot.oops.Klass {
  static {
  ...
  private static synchronized void initialize(TypeDataBase db) throws WrongTypeException {
    ...
    javaFieldsCount      = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_java_fields_count"), 0);
    constants            = new MetadataField(type.getAddressField("_constants"), 0);
    classLoaderData      = type.getAddressField("_class_loader_data");
    sourceDebugExtension = type.getAddressField("_source_debug_extension");
    innerClasses         = type.getAddressField("_inner_classes");
    sourceFileNameIndex  = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_source_file_name_index"), 0);
    nonstaticFieldSize   = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_nonstatic_field_size"), 0);
    staticFieldSize      = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_static_field_size"), 0);
    staticOopFieldCount  = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_static_oop_field_count"), 0);
    nonstaticOopMapSize  = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_nonstatic_oop_map_size"), 0);
    isMarkedDependent    = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_is_marked_dependent"), 0);
    initState            = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_init_state"), 0);
    vtableLen            = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_vtable_len"), 0);
    itableLen            = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_itable_len"), 0);
    breakpoints          = type.getAddressField("_breakpoints");
    genericSignatureIndex = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_generic_signature_index"), 0);
    majorVersion         = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_major_version"), 0);
    minorVersion         = new sun.jvm.hotspot.oops.CIntField(type.getCIntegerField("_minor_version"), 0);
    headerSize           = sun.jvm.hotspot.oops.Oop.alignObjectOffset(type.getSize());
  ...
  public long getObjectSize(sun.jvm.hotspot.oops.Oop object) {
    return getSizeHelper() * VM.getVM().getAddressSize();
  }

  public long getSize() {
    return sun.jvm.hotspot.oops.Oop.alignObjectSize(getHeaderSize() + sun.jvm.hotspot.oops.Oop.alignObjectOffset(getVtableLen()) +
                               sun.jvm.hotspot.oops.Oop.alignObjectOffset(getItableLen()) + sun.jvm.hotspot.oops.Oop.alignObjectOffset(getNonstaticOopMapSize()));
  }
  ...
  public sun.jvm.hotspot.oops.Symbol getFieldName(int index) {
  ...
  public sun.jvm.hotspot.oops.Symbol getFieldSignature(int index) {
  ...
  public short getFieldGenericSignatureIndex(int index) {
    ...
    for (int i = 0; i < allFieldsCount; i++) {
      ...
      sun.jvm.hotspot.oops.AccessFlags access = new sun.jvm.hotspot.oops.AccessFlags(flags);
      if (i == index) {
  ...
  public sun.jvm.hotspot.oops.Symbol getFieldGenericSignature(int index) {
  ...
  public sun.jvm.hotspot.oops.Klass getArrayKlasses()        { return (sun.jvm.hotspot.oops.Klass)        arrayKlasses.getValue(this); }
  ...
  public sun.jvm.hotspot.oops.ConstantPool getConstants()        { return (sun.jvm.hotspot.oops.ConstantPool) constants.getValue(this); }
  ...
  public sun.jvm.hotspot.oops.Oop getClassLoader()         { return                getClassLoaderData().getClassLoader(); }
  public sun.jvm.hotspot.oops.Symbol getSourceFileName()      { return                getConstants().getSymbolAt(sourceFileNameIndex.getValue(this)); }
    ...
    if (length > 0) {
       ...
       for (int i = 0; i < length; i += InnerClassAttributeOffset.innerClassNextOffset) {
          ...
          if (ioff != 0) {
             ...
             sun.jvm.hotspot.oops.ConstantPool.CPSlot classInfo = getConstants().getSlotAt(ioff);
             sun.jvm.hotspot.oops.Symbol name = null;
  ...
  public boolean isInnerClassName(sun.jvm.hotspot.oops.Symbol sym) {
    return isInInnerClasses(sym, false);
  }

  // whether given Symbol is name of an inner/nested Klass of this Klass?
  // anonymous classes excluded, but local classes are included.
  public boolean isInnerOrLocalClassName(sun.jvm.hotspot.oops.Symbol sym) {
    return isInInnerClasses(sym, true);
  }

  private boolean isInInnerClasses(sun.jvm.hotspot.oops.Symbol sym, boolean includeLocals) {
    U2Array innerClassList = getInnerClasses();
    int length = ( innerClassList == null)? 0 : (int) innerClassList.length();
    if (length > 0) {
       if (Assert.ASSERTS_ENABLED) {
       ...
       for (int i = 0; i < length; i += InnerClassAttributeOffset.innerClassNextOffset) {
         ...
         // refer to JVM spec. section 4.7.5.
         if (ioff != 0) {
            sun.jvm.hotspot.oops.ConstantPool.CPSlot iclassInfo = getConstants().getSlotAt(ioff);
            sun.jvm.hotspot.oops.Symbol innerName = getConstants().getKlassNameAt(ioff);
            sun.jvm.hotspot.oops.Symbol myname = getName();
            ...
            if (ooff == 0) {
               ...
               sun.jvm.hotspot.oops.ConstantPool.CPSlot oclassInfo = getConstants().getSlotAt(ooff);
               sun.jvm.hotspot.oops.Symbol outerName = null;
  ...
  public boolean implementsInterface(sun.jvm.hotspot.oops.Klass k) {
    if (Assert.ASSERTS_ENABLED) {
  ...
  boolean computeSubtypeOf(sun.jvm.hotspot.oops.Klass k) {
    if (k.isInterface()) {
   ...
   */
  public void iterateStaticFields(sun.jvm.hotspot.oops.OopVisitor visitor) {
  ...
  void iterateStaticFieldsInternal(sun.jvm.hotspot.oops.OopVisitor visitor) {
    int length = getJavaFieldsCount();
    for (int index = 0; index < length; index++) {
      short accessFlags    = getFieldAccessFlags(index);
      sun.jvm.hotspot.oops.FieldType type   = new sun.jvm.hotspot.oops.FieldType(getFieldSignature(index));
      sun.jvm.hotspot.oops.AccessFlags access = new sun.jvm.hotspot.oops.AccessFlags(accessFlags);
      if (access.isStatic()) {
  ...
  public static class StaticField {
    ...
    StaticField(sun.jvm.hotspot.oops.Field field, sun.jvm.hotspot.oops.AccessFlags flags) {
      this.field = field;
      this.flags = flags;
    }
  }
  ...
  public sun.jvm.hotspot.oops.Field[] getStaticFields() {
    ...
    for (int index = 0; index < length; index++) {
      sun.jvm.hotspot.oops.Field f = newField(index);
      if (f.isStatic()) {
        result.add(f);
      }
    }
    return (sun.jvm.hotspot.oops.Field[])result.toArray(new sun.jvm.hotspot.oops.Field[result.size()]);
  }

  public void iterateNonStaticFields(sun.jvm.hotspot.oops.OopVisitor visitor, sun.jvm.hotspot.oops.Oop obj) {
    if (getSuper() != null) {
  ...
  public sun.jvm.hotspot.oops.Field findLocalField(sun.jvm.hotspot.oops.Symbol name, sun.jvm.hotspot.oops.Symbol sig) {
    int length = getJavaFieldsCount();
    for (int i = 0; i < length; i++) {
      sun.jvm.hotspot.oops.Symbol f_name = getFieldName(i);
      sun.jvm.hotspot.oops.Symbol f_sig  = getFieldSignature(i);
  ...
  public sun.jvm.hotspot.oops.Field findInterfaceField(sun.jvm.hotspot.oops.Symbol name, sun.jvm.hotspot.oops.Symbol sig) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      sun.jvm.hotspot.oops.Field f = intf1.findLocalField(name, sig);
      if (f != null) {
        if (Assert.ASSERTS_ENABLED) {
  ...
  public sun.jvm.hotspot.oops.Field findField(sun.jvm.hotspot.oops.Symbol name, sun.jvm.hotspot.oops.Symbol sig) {
    // search order according to newest JVM spec (5.4.3.2, p.167).
    // 1) search for field in current klass
    sun.jvm.hotspot.oops.Field f = findLocalField(name, sig);
    if (f != null) return f;
  ...
  public sun.jvm.hotspot.oops.Field findField(String name, String sig) {
    SymbolTable symbols = VM.getVM().getSymbolTable();
    sun.jvm.hotspot.oops.Symbol nameSym = symbols.probe(name);
    sun.jvm.hotspot.oops.Symbol sigSym  = symbols.probe(sig);
  ...
  public sun.jvm.hotspot.oops.Field findFieldDbg(String name, String sig) {
    return findField(name, sig);
  }

  /** Get field by its index in the fields array. Only designed for
      use in a debugging system. */
  public sun.jvm.hotspot.oops.Field getFieldByIndex(int fieldIndex) {
  ...
  public sun.jvm.hotspot.oops.Klass arrayKlassImpl(boolean orNull, int n) {
  ...
  public sun.jvm.hotspot.oops.Klass arrayKlassImpl(boolean orNull) {
  ...
  /** Convenience routine taking Strings; lookup is done in
      SymbolTable. */
  public sun.jvm.hotspot.oops.Method findMethod(String name, String sig) {
    SymbolTable syms = VM.getVM().getSymbolTable();
    sun.jvm.hotspot.oops.Symbol nameSym = syms.probe(name);
    sun.jvm.hotspot.oops.Symbol sigSym  = syms.probe(sig);
    if (nameSym == null || sigSym == null) {
      return null;
    }
  ...
  public sun.jvm.hotspot.oops.Method findMethod(sun.jvm.hotspot.oops.Symbol name, sun.jvm.hotspot.oops.Symbol sig) {
  ...
  private void visitField(OopVisitor visitor, sun.jvm.hotspot.oops.FieldType type, int index) {
    sun.jvm.hotspot.oops.Field f = newField(index);
    if (type.isOop()) {
      visitor.doOop((sun.jvm.hotspot.oops.OopField) f, false);
      return;
    }
    if (type.isByte()) {
      visitor.doByte((sun.jvm.hotspot.oops.ByteField) f, false);
      return;
    }
    if (type.isChar()) {
      visitor.doChar((sun.jvm.hotspot.oops.CharField) f, false);
      return;
    }
    if (type.isDouble()) {
      visitor.doDouble((sun.jvm.hotspot.oops.DoubleField) f, false);
      return;
    }
    if (type.isFloat()) {
      visitor.doFloat((sun.jvm.hotspot.oops.FloatField) f, false);
      return;
    }
    if (type.isInt()) {
      visitor.doInt((sun.jvm.hotspot.oops.IntField) f, false);
      return;
    }
    if (type.isLong()) {
      visitor.doLong((sun.jvm.hotspot.oops.LongField) f, false);
      return;
    }
    if (type.isShort()) {
      visitor.doShort((sun.jvm.hotspot.oops.ShortField) f, false);
      return;
    }
    if (type.isBoolean()) {
      visitor.doBoolean((sun.jvm.hotspot.oops.BooleanField) f, false);
      return;
    }
  }

  // Creates new field from index in fields TypeArray
  private sun.jvm.hotspot.oops.Field newField(int index) {
    sun.jvm.hotspot.oops.FieldType type = new FieldType(getFieldSignature(index));
    if (type.isOop()) {
     if (VM.getVM().isCompressedOopsEnabled()) {
        return new NarrowOopField(this, index);
     } else {
        return new sun.jvm.hotspot.oops.OopField(this, index);
     }
    }
    if (type.isByte()) {
      return new sun.jvm.hotspot.oops.ByteField(this, index);
    }
    if (type.isChar()) {
      return new sun.jvm.hotspot.oops.CharField(this, index);
    }
    if (type.isDouble()) {
      return new sun.jvm.hotspot.oops.DoubleField(this, index);
    }
    if (type.isFloat()) {
      return new sun.jvm.hotspot.oops.FloatField(this, index);
    }
    if (type.isInt()) {
      return new sun.jvm.hotspot.oops.IntField(this, index);
    }
    if (type.isLong()) {
      return new sun.jvm.hotspot.oops.LongField(this, index);
    }
    if (type.isShort()) {
      return new sun.jvm.hotspot.oops.ShortField(this, index);
    }
    if (type.isBoolean()) {
      return new sun.jvm.hotspot.oops.BooleanField(this, index);
    }
    throw new RuntimeException("Illegal field type at index " + index);
  }

  private static sun.jvm.hotspot.oops.Method findMethod(MethodArray methods, sun.jvm.hotspot.oops.Symbol name, sun.jvm.hotspot.oops.Symbol signature) {
    ...
    while (l <= h) {
      int mid = (l + h) >> 1;
      sun.jvm.hotspot.oops.Method m = methods.at(mid);
      ...
      if (res == 0) {
        ...
        int i;
        for (i = mid - 1; i >= l; i--) {
          sun.jvm.hotspot.oops.Method m1 = methods.at(i);
  ...
  private static int linearSearch(MethodArray methods, sun.jvm.hotspot.oops.Symbol name, Symbol signature) {
  ...
  public void dumpReplayData(PrintStream out) {
    ...
    sun.jvm.hotspot.oops.Klass sub = getSubklassKlass();
    while (sub != null) {
    ...
    out.println();
    if (isInitialized()) {
      sun.jvm.hotspot.oops.Field[] staticFields = getStaticFields();
      for (int i = 0; i < staticFields.length; i++) {
        Field f = staticFields[i];
        sun.jvm.hotspot.oops.Oop mirror = getJavaMirror();
        if (f.isFinal() && !f.hasInitialValue()) {
          out.print("staticfield " + getName().asString() + " " +
                    sun.jvm.hotspot.oops.OopUtilities.escapeString(f.getID().getName()) + " " +
                    f.getFieldType().getSignature().asString() + " ");
          if (f instanceof sun.jvm.hotspot.oops.ByteField) {
            sun.jvm.hotspot.oops.ByteField bf = (ByteField)f;
            out.println(bf.getValue(mirror));
          } else if (f instanceof sun.jvm.hotspot.oops.BooleanField) {
            sun.jvm.hotspot.oops.BooleanField bf = (BooleanField)f;
            out.println(bf.getValue(mirror) ? 1 : 0);
          } else if (f instanceof sun.jvm.hotspot.oops.ShortField) {
            sun.jvm.hotspot.oops.ShortField bf = (ShortField)f;
            out.println(bf.getValue(mirror));
          } else if (f instanceof sun.jvm.hotspot.oops.CharField) {
            sun.jvm.hotspot.oops.CharField bf = (CharField)f;
            out.println(bf.getValue(mirror) & 0xffff);
          } else if (f instanceof sun.jvm.hotspot.oops.IntField) {
            sun.jvm.hotspot.oops.IntField bf = (IntField)f;
            out.println(bf.getValue(mirror));
          } else  if (f instanceof sun.jvm.hotspot.oops.LongField) {
            sun.jvm.hotspot.oops.LongField bf = (LongField)f;
            out.println(bf.getValue(mirror));
          } else if (f instanceof sun.jvm.hotspot.oops.FloatField) {
            sun.jvm.hotspot.oops.FloatField bf = (FloatField)f;
            out.println(Float.floatToRawIntBits(bf.getValue(mirror)));
          } else if (f instanceof sun.jvm.hotspot.oops.DoubleField) {
            sun.jvm.hotspot.oops.DoubleField bf = (DoubleField)f;
            out.println(Double.doubleToRawLongBits(bf.getValue(mirror)));
          } else if (f instanceof sun.jvm.hotspot.oops.OopField) {
            sun.jvm.hotspot.oops.OopField bf = (OopField)f;

            Oop value = bf.getValue(mirror);
            if (value == null) {
              out.println("null");
            } else if (value.isInstance()) {
              sun.jvm.hotspot.oops.Instance inst = (Instance)value;
