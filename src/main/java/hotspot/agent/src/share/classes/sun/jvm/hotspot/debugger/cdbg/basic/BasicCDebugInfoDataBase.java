...
public class BasicCDebugInfoDataBase implements CDebugInfoDataBase {
  ...
  public void resolve(sun.jvm.hotspot.debugger.cdbg.basic.ResolveListener listener) {
    if (Assert.ASSERTS_ENABLED) {
      Assert.that(state == CONSTRUCTION_STATE, "wrong state");
    }
    ...
    for (ListIterator iter = types.listIterator(); iter.hasNext(); ) {
      sun.jvm.hotspot.debugger.cdbg.basic.BasicType t = (sun.jvm.hotspot.debugger.cdbg.basic.BasicType) iter.next();
      sun.jvm.hotspot.debugger.cdbg.basic.BasicType t2 = (sun.jvm.hotspot.debugger.cdbg.basic.BasicType) t.resolveTypes(this, listener);
      if (t != t2) {
        iter.set(t2);
      }
    }
    ...
    for (Iterator iter = blocks.iterator(); iter.hasNext(); ) {
      ((sun.jvm.hotspot.debugger.cdbg.basic.BasicSym) iter.next()).resolve(this, listener);
    }
  ...
  public Type lookupType(String name, int cvAttributes) {
    ...
    sun.jvm.hotspot.debugger.cdbg.basic.BasicType t = (sun.jvm.hotspot.debugger.cdbg.basic.BasicType) nameToTypeMap.get(name);
    if (t != null) {
      if (cvAttributes != 0) {
        t = (sun.jvm.hotspot.debugger.cdbg.basic.BasicType) t.getCVVariant(cvAttributes);
      }
    }
    return t;
  }
  ...
  public Type resolveType(Type containingType, Type targetType, sun.jvm.hotspot.debugger.cdbg.basic.ResolveListener listener, String detail) {
    sun.jvm.hotspot.debugger.cdbg.basic.BasicType basicTargetType = (sun.jvm.hotspot.debugger.cdbg.basic.BasicType) targetType;
    if (Assert.ASSERTS_ENABLED) {
      Assert.that(state == CONSTRUCTION_STATE, "wrong state");
    }
    if (basicTargetType.isLazy()) {
      sun.jvm.hotspot.debugger.cdbg.basic.BasicType resolved = (sun.jvm.hotspot.debugger.cdbg.basic.BasicType) lazyTypeMap.get(((sun.jvm.hotspot.debugger.cdbg.basic.LazyType) targetType).getKey());
      ...
      if (resolved == null) {
        listener.resolveFailed(containingType, (sun.jvm.hotspot.debugger.cdbg.basic.LazyType) targetType, detail + " because target type was not found");
        return targetType;
      }
      if (resolved.isLazy()) {
        // Might happen for const/var variants for forward references
        if (resolved.isConst() || resolved.isVolatile()) {
          resolved = (sun.jvm.hotspot.debugger.cdbg.basic.BasicType) resolved.resolveTypes(this, listener);
        }
        if (resolved.isLazy()) {
          listener.resolveFailed(containingType, (sun.jvm.hotspot.debugger.cdbg.basic.LazyType) targetType,
                                 detail + " because target type (with key " +
                                 ((Integer) ((sun.jvm.hotspot.debugger.cdbg.basic.LazyType) resolved).getKey()).intValue() +
  ...
  public Type resolveType(Sym containingSymbol, Type targetType, sun.jvm.hotspot.debugger.cdbg.basic.ResolveListener listener, String detail) {
    sun.jvm.hotspot.debugger.cdbg.basic.BasicType basicTargetType = (sun.jvm.hotspot.debugger.cdbg.basic.BasicType) targetType;
    if (Assert.ASSERTS_ENABLED) {
      Assert.that(state == CONSTRUCTION_STATE, "wrong state");
    }
    if (basicTargetType.isLazy()) {
      ...
      if (resolved == null) {
        listener.resolveFailed(containingSymbol, (sun.jvm.hotspot.debugger.cdbg.basic.LazyType) targetType, detail);
        return targetType;
      }
      if (resolved.isLazy()) {
        // Might happen for const/var variants for forward references
  ...
  /** Intended only to be usd by the BasicSym implementation. */
  public Sym resolveSym(Sym containingSymbol, Sym targetSym, sun.jvm.hotspot.debugger.cdbg.basic.ResolveListener listener, String detail) {
    if (targetSym == null) return null;
    sun.jvm.hotspot.debugger.cdbg.basic.BasicSym basicTargetSym = (sun.jvm.hotspot.debugger.cdbg.basic.BasicSym) targetSym;
    if (Assert.ASSERTS_ENABLED) {
      Assert.that(state == CONSTRUCTION_STATE, "wrong state");
    }
    if (basicTargetSym.isLazy()) {
      sun.jvm.hotspot.debugger.cdbg.basic.BasicSym resolved = (BasicSym) lazySymMap.get(((sun.jvm.hotspot.debugger.cdbg.basic.LazyBlockSym) targetSym).getKey());
      // FIXME: would like to have an assert here that the target is
      // non-null, but apparently have bugs here
      if (resolved == null) {
        listener.resolveFailed(containingSymbol, (sun.jvm.hotspot.debugger.cdbg.basic.LazyBlockSym) targetSym, detail);
        return targetSym;
      }
      if (resolved.isLazy()) {
  ...
  private void resolveLazyMap(ResolveListener listener) {
    for (Iterator iter = lazyTypeMap.entrySet().iterator(); iter.hasNext(); ) {
      ...
      sun.jvm.hotspot.debugger.cdbg.basic.BasicType t = (sun.jvm.hotspot.debugger.cdbg.basic.BasicType) entry.getValue();
      sun.jvm.hotspot.debugger.cdbg.basic.BasicType t2 = (BasicType) t.resolveTypes(this, listener);
      if (t2 != t) {
        entry.setValue(t2);
      }
    }
